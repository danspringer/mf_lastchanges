...
<form id="rex-addon-editmode" action="index.php?page=mf_lastchanges/lastchanges_media&func=filter" method="post">
<fieldset>
...
if ($func == '' && $username == '') {
	//echo '0';
	$list = rex_list::factory("SELECT id, title, filename,  updatedate, updateuser FROM `".rex::getTablePrefix()."media` ORDER BY `updatedate` DESC", $anzahl);
	}
	
// Nach Filternwendung und Paginierungs-Klick
if ($func == '' && $username != '' && in_array($username, $userarray) ) {
	//echo '1';
	$list = rex_list::factory("SELECT id, title, filename,  updatedate, updateuser, category_id FROM `".rex::getTablePrefix()."media` WHERE updateuser = '$username' ORDER BY `updatedate` DESC", $anzahl, $username);
	}
	
// Nach Filternwendung und Paginierungs-Klick
if ($func == '' && $username != '' && !in_array($username, $userarray) ) {
	//echo '2';
	$list = rex_list::factory("SELECT id, title, filename,  updatedate, updateuser, category_id FROM `".rex::getTablePrefix()."media` ORDER BY `updatedate` DESC", $anzahl);
	}
...
if ($func == 'filter' ) {
	...
	$list->setColumnParams('id', ['page' => 'mediapool', 'file_id'=> '###id###', 'rex_file_category' => '###category_id###']);
	$list->addLinkAttribute( 'id', 'target', '_blank' );
